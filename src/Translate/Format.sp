...
chainPrecedence as fn [ FN.Binop & a ]: Int =
    fn ls:
    try ls as
        , []: 0
...
expressionPrecedence as fn FN.Expression: Int =
    fn FN.Expression _ _ e_:
    try e_ as

        , FN.Statements stats:
            0

        , FN.BinopChain priority binopChain:
            chainPrecedence binopChain.second

        , FN.Poly text expression:
            9
        ...
        , FN.Lowercase { with  maybeType = Just _ }:
...
formatExpressionAndMaybeAddParens as fn Env, Int, FN.Expression: Fmt.Block =
...
formatExpression as fn Env, FN.Expression: Fmt.Block =
    fn env, faExpression:
    FN.Expression comments _ e_ =
        faExpression

    try e_ as

        , FN.LiteralText singleOrTriple text:
            formatLiteralText singleOrTriple text

        , FN.LiteralNumber hasPercentage text:
            formatLiteralNumber hasPercentage text

        , FN.ArgumentPlaceholder:
            Fmt.textToBlock "__"

        , FN.Statements stats:
            formatStatements env stats

        , FN.List isMultiline unpacksAndExprs:
            formatList env isMultiline unpacksAndExprs

        , FN.Record { attrs, isMultiline, maybeExtension }:
            formatRecord env isMultiline maybeExtension attrs

        , FN.Lowercase { attrPath, maybeModule, maybeType, name }:
            formatLowercase env maybeType maybeModule name attrPath

        , FN.Uppercase { maybeModule, name }:
            formatUppercase env maybeModule name

        , FN.Constructor { maybeModule, name }:
            formatConstructor env maybeModule name

        , FN.RecordShorthand { attrPath, name }:
            formatRecordShorthand env name attrPath

        , FN.Fn layout pars body:
            formatFunction env layout pars body

        , FN.UnopCall unopId expr:
        ...
        , FN.If { condition, false, isMultiline, true }:
            formatIf env isMultiline faExpression

        , FN.Try { patterns, value }:
...
formatComment as fn Env, FN.Comment: Fmt.Block =
...
formatComments as fn Env, [ FN.Comment ]: Fmt.Block =
    fn env, comments:
    comments
...
extractComments as fn FN.Expression: FN.Expression & [ FN.Comment ] =
    fn FN.Expression comments pos expr:
    FN.Expression [] pos expr & comments


stackWithComments as fn Env, [ FN.Comment ], Fmt.Block: Fmt.Block =
...
formatStatements as fn Env, [ FN.Statement ]: Fmt.Block =
    fn env, sss:
    rec =
...
formatStatement as fn Env, FN.Statement: Fmt.Block =
    fn env, stat:
    try stat as
        , FN.CommentStatement comment: formatComment env comment
        , FN.Evaluation expression: formatExpression env expression
        , FN.ValueDef valueDef: formatValueDef env valueDef
        , FN.AliasDef aliasDef: formatAliasDef env aliasDef
        , FN.UnionDef unionDef: formatUnionDef env unionDef


formatValueDef as fn Env, FN.ValueDef: Fmt.Block =
...
formatAliasDef as fn Env, FN.AliasDef: Fmt.Block =
...
formatUnionDef as fn Env, FN.UnionDef: Fmt.Block =
...
formatList as fn Env, Bool, [ Bool & FN.Expression ]: Fmt.Block =
...
formatRecord as fn Env, Bool, Maybe (Maybe FN.Expression), [ FN.RecordAttribute ]: Fmt.Block =
    ...
    formatRecordAttribute as fn FN.RecordAttribute: Fmt.Block =
        fn { maybeExpr, name }:
        try maybeExpr as
    ...
    attributeName =
        ...
        try nameExpr.name as
            , FN.Expression _ _ (FN.Lowercase { with name }): name
            , _: ""
...
formatLowercase as fn Env, Maybe FN.Expression, Maybe Name, Name, [ Name ]: Fmt.Block =
...
formatFunctionHeader as fn Env, [ FN.Expression ]: Fmt.Block =
    fn env, pars:
    pars
...
formatFunction as fn Env, FN.Layout, [ FN.Expression ], FN.Expression: Fmt.Block =
...
formatUnopCall as fn Env, Op.UnopId, FN.Expression: Fmt.Block =
    fn env, unopId, expr:
    unop =
        unopToText unopId
...
formatBinopChain as fn Env, Int, FN.BinopChain: Fmt.Block =
...
formatCall as fn Env, FN.Expression, [ FN.Expression ]: Fmt.Block =
    fn env, ref, args:
    lastIndex =
        List.length args - 1
    ...
    asContinuingFn =
        ...
        try arg as
            ...
            , FN.Expression _ _ (FN.Fn layout params body):
    ...
    formatArgument as fn Int, FN.Expression: Fmt.Block =
        fn index, arg:
...
extractIfElses as fn Env, FN.Expression: [ [ FN.Comment ] & Fmt.Block & Fmt.Block ] & Fmt.Block =
    fn env, x:
    rec =
        fn acc, expr:
        try expr as
            , FN.Expression comments _ (FN.If args): rec [ comments & formatExpression env args.condition & formatExpression env args.true, ...acc ] args.false
            , _: List.reverse acc & formatExpression env expr

    rec [] x


formatIf as fn Env, Bool, FN.Expression: Fmt.Block =
    ...
    try maybeSingleLine as
        ...
        , Nothing:
            formatCAndV as fn Int, [ FN.Comment ] & Fmt.Block & Fmt.Block: Fmt.Block =
...
formatTry as fn Env, FN.Expression, [ FN.Expression & FN.Expression ]: Fmt.Block =
