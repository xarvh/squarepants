...
resolve getEdges target state0 =
    as (key: Set key): key: State key: Result [key] (State key)
    with key NonFunction

    if List.member target state0.resolved:
        Ok state0

    else if Set.member target state0.unresolved:
        Err [ target ]
        ...
        try Dict.foldlRes (fn a None: resolve getEdges a) (getEdges target) s as
            Err path:
                Err << target :: path
...
reorder nodeToEdges nodesById =
    ...
    , resolved = []
        ...
        >> Dict.foldlRes (fn k v: resolve keyToEdges k) nodesById
        >> Result.map (fn x: x.resolved >> List.filterMap (fn id: Dict.get id nodesById) >> List.reverse)
