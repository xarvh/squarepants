            ...
            try ga pos usr as
                ...
                Ok (CA.TypeDefUnion un):
                    if List.length replacedArgs /= List.length un.args then
                        error pos [
                            , "union " .. SPCore.toHuman un.usr .. " needs " .. Text.fromNumber (List.length un.args) .. " args,"
                ...
                Ok (CA.TypeDefAlias al):
                    if List.length al.args /= List.length replacedArgs then
                        error pos [ "alias " .. SPCore.toHuman al.usr .. " needs " .. Text.fromNumber (List.length al.args) .. " args, but was used with " .. Text.fromNumber (List.length replacedArgs) ]
    ...
    try findMutableArgsThatContainFunctions Nothing expandedTy as
        ...
        errors:
            ...
            error (Pos.I 567)
                ...
                , errors >> List.map SPCore.toHuman >> Text.join "\n"
                ]
        ...
        try Dict.get usr expandedTypes as
            ...
            Nothing:
                try Dict.get usr allTypes as
                    Nothing:
                        error pos [ "Undefined type: `" .. SPCore.toHuman usr .. "`" ]

                    Just (CA.TypeDefAlias a):
                        SPCore.todo << "expandAndInsertAlias should-not-happen: " .. SPCore.toHuman usr
    ...
    try Dict.get usr expandedTypes as
        ...
        Nothing:
            ...
            try Dict.get usr allTypes as
                ...
                Nothing:
                    error pos [ "Undefined type usr: `" .. SPCore.toHuman usr .. "`" ]
...
expandAnnotation as CA.All CA.TypeDef: CA.Type: Res CA.Type =
    ...
    gt as GetType =
        ...
        try Dict.get usr allExpandedTypes as
            Just t: Ok t
            Nothing: error pos [ "Undefined type usr: `" .. SPCore.toHuman usr .. "`" ]
